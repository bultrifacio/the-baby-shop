...
export const FilterBar: React.FunctionComponent<SortSearchBarProps> = (props) => {
    ...
    const FilterButton: React.FunctionComponent = () => {
        ...
        return (
            <div className="filter-bar-item drawer-menu-button">
                <Button type="primary" onClick={() => onClickDrawerMenu(true)}>
                    {intl.formatMessage({ id: 'filter.bar.button.menu' })}
                </Button>
            </div>
        );
    };
    ...
    const SelectDirection: React.FunctionComponent = () => {
        return (
            <div className="filter-bar-item">
                ...
                >
                    <Option value={DirectionEnum.ASC}>
                        {intl.formatMessage({ id: 'filter.bar.select.direction.asc' })}
                    </Option>
                    <Option value={DirectionEnum.DESC}>
                        {intl.formatMessage({ id: 'filter.bar.select.direction.desc' })}
                    </Option>
                </Select>
            </div>
        );
        ...
        return (
            <div className="filter-bar-item">
                ...
                >
                    ...
                    <Option value={OrderEnum.PRICE}>
                        {intl.formatMessage({ id: 'filter.bar.select.order.price' })}
                    </Option>
                    <Option value={OrderEnum.BESTSELLERS}>
                        {intl.formatMessage({ id: 'filter.bar.select.order.bestseller' })}
                    </Option>
                </Select>
            </div>
        );
    ...
    const SearchBar: React.FunctionComponent = () => {
        return (
            <div className="filter-bar-item">
                <Search
                    placeholder={intl.formatMessage({ id: 'filter.bar.search.input.placeholder' })}
