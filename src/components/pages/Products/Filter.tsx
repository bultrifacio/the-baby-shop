...
export const Filter: React.FunctionComponent<SearchFilterProps> = (props) => {
        ...
        return (
            ...
            >
                {composition.map((composition: string) => (
                    <Option key={shortid.generate()} value={composition}>
                        {composition}
                    </Option>
                ))}
            </Select>
        ...
        return (
            ...
            >
                {colors.map((color: string) => (
                    <Option key={shortid.generate()} value={color}>
                        {color}
                    </Option>
                ))}
            </Select>
        ...
        return (
            ...
            >
                {sizes.map((size: ProductVariant) => (
                    <Option key={shortid.generate()} value={size.variantId}>
                        {size.name}
                    </Option>
                ))}
            </Select>
    ...
    const ClearButton: React.FunctionComponent = () => (
        <Button onClick={onClickClearFilters} icon={<CloseOutlined />}>
            {intl.formatMessage({ id: 'filter.clear.button' })}
        </Button>
    );
