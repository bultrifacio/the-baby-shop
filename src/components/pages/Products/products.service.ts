...
export const getCategories = (storeViewId: string): Promise<Array<Category>> => {
    ...
    return axios.get(`${API_URL!}/stores/${storeViewId}/categories`).then((response: AxiosResponse) => response.data);
};
...
export const getProducts = (storeViewId: string, filtersPayload?: FilterPayload): Promise<ProductSearch> => {
    ...
    return axios
        .get(
            `${API_URL!}/stores/${storeViewId}/products/search?${filters}&${withText}&${categoryId}&${order}&${dir}&${page}&${limit}`,
        )
