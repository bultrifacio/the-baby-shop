...
export const Products: React.FunctionComponent<ProductSearchPageProps> = (props) => {
    ...
    React.useEffect(() => {
        if (failureCountCategories === 1) message.error(intl.formatMessage({ id: 'products.request.error.categories' }));
    }, [failureCountCategories, intl]);
    ...
    React.useEffect(() => {
        if (failureCountProducts === 1) message.error(intl.formatMessage({ id: 'products.request.error.products' }));
    }, [failureCountProducts, intl]);
    ...
    const obtainSizes = (products: Array<Product>): Array<ProductVariant> =>
        uniqBy(
            flatten(products.map((product: Product) => product.sizes.map((size: ProductVariant) => size))),
            'variantId',
        );
